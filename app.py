...
def upload_files():
    ...
    if pdf_paths:
        ...
        output_buffer = combine_dataframes_to_excel(processed_dataframes)
...
def process_single_pdf(pdf_file):
    ...
    try:
        ...
        print(f"{pdf_file.filename} verwerkt in {time.time() - start_time:.2f} seconden")
...
def combine_dataframes_to_excel(dataframes):
    ...
    for idx, df in enumerate(dataframes):
        if not df.empty:
            ...
            for r_idx, row in enumerate(df.itertuples(index=False), start=1):
                ...
                ws.cell(row=r_idx, column=8).fill = yellow_fill
                ws.cell(row=r_idx, column=9).fill = blue_fill
